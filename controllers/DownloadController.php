 ...
 */
class DownloadProtect_DownloadController extends Omeka_Controller_AbstractActionController
{
    protected $_type;
    protected $_storage;
    protected $_filename;
    ...
    public function confirmAction()
    {
        ...
        $form = new DownloadProtect_Form_Confirm();
        $this->view->form = $form;
    ...
    public function sendAction()
    {
        ...
        $this->view->sendUrl = WEB_ROOT . '/download-protect/download/send';
        $this->view->source_page = $this->session->sourcePage;
    ...
    protected function _getToConfirm()
    {
        if (is_null($this->_toConfirm)) {
            ...
            // Check for captcha;
                ...
                $filesize = $this->_getFilesize();
                $this->_toConfirm = ($filesize > (int) get_option('download_protect_max_free_download'));
            }
        }
    ...
    protected function _getMode()
    {
        if (is_null($this->_mode)) {
            ...
            switch ($mode) {
                ...
                case 'size':
                    $filesize = $this->_getFilesize();
                    $this->_mode = ($filesize > (int) get_option('download_protect_max_free_download'))
                        ? 'attachment'
                        : 'inline';
                    break;
                ...
                case 'image-size':
                    ...
                    $contentType = $this->_getContentType();
                    $this->_mode = (strpos($contentType, 'image') === false
                            || $filesize > (int) get_option('download_protect_max_free_download'))
                        ? 'attachment'
                        : 'inline';
                    break;
